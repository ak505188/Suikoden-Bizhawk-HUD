...
for _,name in ipairs(CharacterNames) do
  ...
  if character.recruit_address then
...
function Menu:updateRecruitmentData()
  ...
  for _,character in ipairs(RecruitableCharacters) do
    local recruit_value_position = character.recruit_address - Recruit_First_Slot_Address + 1
...
function Menu:changeRecruitValue(amount)
  ...
  memory.write_u8(character.recruit_address, new_recruit_value)
end
